...
function ForgotPassword() {
  ...
  const changePassword = (emailAddress, newPass) => {
    ...
    for (let i = 0; i < registeredUsers.length; i++) {
      for (const [email, pass] of Object.entries(registeredUsers[i])) {
        if (email === emailAddress) {
          ...
          // Checking if the old password is the same as the new password.
          if (pass === newPass) {
            alert(
    ...
    // Notifying non-existing users to create a new account.
    if (!emailExists) {
      alert("This user does not exist. Please create a new account.");
    }
  };
